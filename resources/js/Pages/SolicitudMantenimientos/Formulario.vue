...
const { oSolicitudMantenimiento, limpiarSolicitudMantenimiento } = useSolicitudMantenimientos();
const accion = ref(props.accion_dialog);
const dialog = ref(props.open_dialog);
let form = useForm(oSolicitudMantenimiento.value);
watch(
    () => props.open_dialog,
    (newValue) => {
        dialog.value = newValue;
        if (dialog.value) {
            form = useForm(oSolicitudMantenimiento.value);
            cargarUsuarios();
        }
    }
);
...
const tituloDialog = computed(() => {
    return accion.value == 0 ? `Agregar SolicitudMantenimiento` : `Editar SolicitudMantenimiento`;
});
                            ...
                            <v-row>
                                <v-col cols="12" sm="12" md="12">
                                    <v-text-field
                                        ...
                                        "
                                        variant="outlined"
                                        label="Nombre SolicitudMantenimiento*"
                                        required
                                        density="compact"
                                        v-model="form.nombre"
                                    ></v-text-field>
