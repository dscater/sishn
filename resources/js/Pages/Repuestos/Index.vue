...
const { getRepuestosApi, setRepuesto, limpiarRepuesto, deleteRepuesto } =
    useRepuestos();
const responseRepuestos = ref([]);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
    ...
    setTimeOutLoadData = setTimeout(async () => {
        responseRepuestos.value = await getRepuestosApi(options.value);
        listRepuestos.value = responseRepuestos.value.data;
        totalItems.value = parseInt(responseRepuestos.value.total);
        loading.value = false;
    }, 300);
};
const recargaRepuestos = async () => {
    loading.value = true;
    listRepuestos.value = [];
    options.value.search = search.value;
    responseRepuestos.value = await getRepuestosApi(options.value);
...
const editarRepuesto = (item) => {
    setRepuesto(item);
    accion_dialog.value = 1;
    open_dialog.value = true;
};
const eliminarRepuesto = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let respuesta = await deleteRepuesto(item.id);
...
<template>
    ...
    <v-container>
        ...
        <Formulario
            ...
            @envio-formulario="recargaRepuestos"
